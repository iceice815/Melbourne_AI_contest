...
class OffensiveReflexAgent(CaptureAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        self.capsules_num = len(self.getCapsules(gameState))
        self.power_step_length = 0
...
class DefensiveReflexAgent(OffensiveReflexAgent):
    ...
    def ice_DefenceAttack(self, gameState):
        ...
        if(len(foods_positions) > len(foods_list)):
            ...
            self.use_attack(gameState)
    def use_astarAttack(self,gameState):
    ...
    def chooseAction(self, gameState):
        ...
        cap_num = len(self.getCapsules(gameState))
        if (cap_num != self.capsules_num):
            print("111")
            if (self.power_step_length > 40):
                self.power_step_length = 0
                self.capsules_num = cap_num
                return self.aStarSearch([self.mid_point])
            else:

                print "333"

                self.power_step_length=self.power_step_length+1
                return self.use_astarAttack(gameState)
        else:
