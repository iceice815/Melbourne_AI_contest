...
class OffensiveReflexAgent(CaptureAgent):
    ...
    def getFeatures(self, gameState, action):
        ...
        if len(self.getCapsules(successor)) > 0:
            # features['getCapsules'] = -min([self.getMazeDistance(
            #     myPos, x) for x in self.getCapsules(successor)])
    ...
    def aStarSearch2(self, gameState, target):
    ...
    def aStarSearchEscape(self, gameState, target):
        ...
        while myPos != target:
            ...
            for action in plan[1].getLegalActions(self.index):
                ...
                for opponent in self.opponents:
                    # print 'emeny'
                    if successor.getAgentPosition(opponent) != None and not (successor.getAgentState(opponent).isPacman):
...
class DefensiveReflexAgent(OffensiveReflexAgent):
    ...
    def ice_DefenceAttack(self, gameState):
        """
                Picks among the actions with the highest Q(s,a).
                """
        #print back = gameState.getAgentState(idx).isPacman
        ...
        back = False
        for idx in self.opponents:
            back = back or gameState.getAgentState(idx).isPacman
        print back

        if back == True :
            self.plan = []
            self.draw = []
            print (self.aStarSearch(self.ice_Defend(gameState)),"123")
